...
def kramer(content: str, key: int) -> str:
    ...
    _final_content_ = fr"""class LeHoangPhuc():
#TácGiảLêHoàngPhúc#   	
...
LeHoangPhuc(_n1_=False,_n2_=False,_sparkle='''{content}''')""".strip().replace("_n1_",n_1_.removeprefix("self.")).replace("_n2_",n_2.removeprefix("self.")).replace("_n3_",n_3.removeprefix("self.")).replace("_n4_",n_4.removeprefix("self.")).replace("_n5_",n_5).replace("_n6_",n_6).replace("_n7_",n_7).replace("_n8_",n_8).replace("_n9_",n_9.removeprefix("self.")).replace("_n10_",n_10.removeprefix("self.")).replace("_n12_",n_12.removeprefix("self."))
    return _final_content_



System.Clear()
System.Title("LeHoangPhuc")
System.Size(140, 45)
...
╚═╝  ╚═╝╚═╝  ╚═══╝     ╚═════╝ ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝    
        ...
        ──────────────────────            
           =>Nhóm Zalo: https://zalo.me/g/dpfbxq529
           =>Youtube: https://youtube.com/@anorintool970
           =>Phiên Bản Tool 2.0
        ──────────────────────
...
def main():
    ...
    _file = Write.Input("Nhập Tên File Pyhon cần encode -> ", Colors.red_to_yellow, interval=0.005)
